function deleteplotRowForDynamicTable(id, row, plot, name) {
		...
		if (confirm("Do you really want to delete this plot") == true) {
			...
			for ( var i = dltRowcount; i < rowlength; i++) {
				var tempRow = parseInt(i) + 1;
				 ...
				 var tempRow = parseInt(i)+1;
				 document.getElementById("deletePlot@"+tempRow).id = "deletePlot@"+i;	
	...
	} else if (confirm("Do you really want to delete this plot") == true){
...
var ownerValues = firstname + "@" + middlename + "@" + lastname+"@"
		...
		+ converBanglaToNumeric(ownerSymbol)+"@"+rowCount+"@"+"NA"+"@"+issharesame;
	  ...
	  if( confirm("Are you sure to submit data?") == true)
	    {
	    	...
	    	document.saveform.action = "saveencodeddata";
...
if(resAddress[2]=="-")
{
	...
	document.getElementById('revenue').value = res[3];
...
document.getElementById('mouzaname').value ="0";
	...
	loadTblProg('mouzaname','getMouzaFromUpazilla',param,resAddress[3]);
	}
...
if(document.getElementById('relationTypeCheck').checked == false && document.getElementById('shareCheck').checked == false )
	...
	+ converBanglaToNumeric(revenue) + "@" + ownershipTypeid + "@" + converBanglaToNumeric(ownershare) + "@"
 + converBanglaToNumeric(ownersharsymbole)+"@"+rowcountidentifyowner+"@"+ ownerid+"@"+issharesame;

	 
		var	isrelationsame="0"+"@"+addressid+"@"+relationtypeid;
		 addressvalue=hno+ "@" + stnm + "@" + locnm + "@" + mouzaid + "@" + upzillaid + "@"
		+ districtid + "@" + relationtypeid + "@" + relationnamevalue+"@"+isrelationsame+"@"+isRelationSameflag;
	...
	resultownervalue[6]="0";
		...
		"@"+resultownervalue[7]+"@"+resultownervalue[8]+"@"+issharesame;
		allValues=editvalue;
		
		relationtypeid=resAddress[6];
		//resAddress[3]=prvaddressid;
		var	isrelationsame="1"+"@"+prvaddressid+"@"+resAddress[6];
		addressvalue=resAddress[0]+"@"+resAddress[1]+"@"+resAddress[2]+"@"+resAddress[3]+"@"+resAddress[4]+"@"+resAddress[5]+"@"+resAddress[6]+"@"+resAddress[7]+"@"+isrelationsame+"@"+"0";
...
if(lastname=='-')
{
	...
	table.rows[rowcountidentifyowner].cells[2].innerHTML = hno + " " + stnm
			...
			;
	table.rows[rowcountidentifyowner].cells[3].innerHTML = revenueText;
...
if(resultownervalue[3]=='')
	{
	resultownervalue[3]="0";
	}
...
addressvalue=values[0]+"@"+values[1]+"@"+values[2]+"@"+values[3]+"@"+values[4]+"@"+values[5]+"@"+values[6]+"@"+values[7]+"@"+isrelationsame+"@"+"0";
...
document.getElementById("relationid@" + i).value = relationId;
document.getElementById("editHiddenownerAddress@" + i).value=addressvalue;
document.getElementById("ownerShipid@" + i).value=converBanglaToNumeric(res[4]);
}
...
+ resultaddress[5] + "@" + resultaddress[6] + "@" + resultaddress[7]+"@"+isrelationsame+"@"+"0";
...
var isrelationsame="1"+"@"+addressId+"@"+resultaddress[6];
	...
	document.getElementById("editHiddenAddress@" + i).value = nextchangedvaluesaddress;
	document.getElementById("addedflag@" + i).value = 'added';
	document.getElementById("relationid@" + i).value = prvrelationid;
	document.getElementById("addressid@" + i).value = addressId;
	document.getElementById("ownerShipid@" + i).value=prvownershipTypeid;
...
function clearPossesionDetails() {
	...
	function ResetEncodingForm(){
		     if (confirm("Are you sure to Reset the Form?") == true) {
					document.saveform.action="DataEncoding";
					document.saveform.submit();
	...
	function checkForPossessionValidation(){
		...
		/*	var possessionOwnerNamevalue = document.getElementById('possessionOwnerName').options[document
			                                                            			    .getElementById('possessionOwnerName').selectedIndex].value;
			 
		 */    
		var	possvales=possOwnerName.split("@");
		var prowcount=possvales[1];
		var pownerid=possvales[2];
			if(possOwnerNo.length >0)
				{
			for(var i=1;i<=possOwnerNo.length;i++)
				{
			     var ownerrow= document.getElementById("prowcount"+i).value.trim();
			     var  posswnerid= document.getElementById("pownerid"+i).value.trim();
			     var plot =document.getElementById("plotSrNo"+i).value.trim();
			     if(pownerid=="NA")
					  {
										  
				  if(ownerrow.trim()==prowcount.trim() && possessionPlotno.trim()==plot.trim())
					  {
					  ...
					  return false;
						  ...
						  {
						  if(posswnerid.trim()==pownerid.trim() && possessionPlotno.trim()==plot.trim())
						  {
